...
import cascading.flow.tez.planner.Hadoop3TezFlowStepJob;
 ...
 *
 */
public class Hadoop3TezFlowStep extends BaseFlowStep<TezConfiguration>
  {
  private static final Logger LOG = LoggerFactory.getLogger( Hadoop3TezFlowStep.class );

  private Map<String, LocalResource> allLocalResources = new HashMap<>();
  private Map<Path, Path> syncPaths = new HashMap<>();
  private Map<String, String> environment = new HashMap<>();

  public Hadoop3TezFlowStep( ElementGraph elementGraph, FlowNodeGraph flowNodeGraph )
    {
  ...
  public TezConfiguration createInitializedConfig( FlowProcess<TezConfiguration> flowProcess, TezConfiguration parentConfig )
    ...
    String flowStagingPath = ( (Hadoop3TezFlow) getFlow() ).getFlowStagingPath();
    List<String> classPath = ( (Hadoop3TezFlow) getFlow() ).getClassPath();
  ...
  protected FlowStepJob createFlowStepJob( ClientState clientState, FlowProcess<TezConfiguration> flowProcess, TezConfiguration initializedStepConfig )
    ...
    return new Hadoop3TezFlowStepJob( clientState, this, initializedStepConfig, dag );
    }
